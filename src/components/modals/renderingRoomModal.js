...
export default function RenderingRoomModal() {
  ...
  return (
    <>
        <div className=" bg-white rounded px-2 py-1 flex items-center text-black">
            ...
            <button onClick={handleLogout} className=" bg-sky-400 rounded-2xl px-2 py-1 ml-auto font-bold text-white hover:bg-sky-500">Logout</button>
        </div>
        ...
        <div className="flex flex-col justify-between mt-1 text-white">
          <button onClick={handleAddRoom} className=" bg-red-500 px-2 py-1 my-2 rounded hover:bg-red-600">create group chat</button>
          <button onClick={handleJoinRoom} className="bg-black px-2 py-1 rounded hover:bg">join group chat</button>
        </div>
    </>
  )
}
