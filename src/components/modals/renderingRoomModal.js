...
function RenderingRoomModal() {
  ...
  return (
    <>
        <div className=" rounded-lg px-2 py-2 mb-3 flex items-center bg-[#F5F8FC] border-2">
        ...
        <ul className=" group-list">
            {rooms.map((room) => (
            <li key={room.id} onClick={() => setSelectedRoomId(room.id)} className={`flex py-2 px-2 mb-1 rounded-large overflow-hidden ${selectedRoomId === room.id ? 'bg-[#eef1f5]' : null}`}>
